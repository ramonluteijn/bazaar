            ...
            <div class="w-full md:w-3/4 p-4">
                ...
                <form action="{{ isset($page) ? route('pages.update', $page->id) : route('pages.store') }}" enctype="multipart/form-data" method="post" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        @if(isset($page))
                            <a href="{{route('pages.read-from-url', $page->url)}}" target="_blank" class="text-blue-500 mb-4 inline-block">View Page</a>
                        @endif
                        <x-forms.input-select :name="'header_font'" label="header font" :list="$fonts" :value="$page->header_font ?? ''"/>
                        <x-forms.input-select :name="'body_font'" :list="$fonts" label="body font" :value="$page->body_font ?? ''"/>
